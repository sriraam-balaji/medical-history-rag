...
function App() {
  ...
  async function askArchive(event: React.FormEvent) {
    ...
    else { const sources = (data?.citations ?? []).map((citation: { document_id: string; page_start: number | null; page_end: number | null }) => `Document ${citation.document_id.slice(0, 8)} · page ${citation.page_start ?? '—'}${citation.page_end && citation.page_end !== citation.page_start ? `–${citation.page_end}` : ''}`).join('\n'); setAnswer(`${data?.answer ?? 'No answer returned.'}${sources ? `\n\nRetrieved sources:\n${sources}` : ''}`); setNotice('Answer generated from indexed records.') }
  }
